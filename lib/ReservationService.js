...
export class ReservationService{
    ...
    async loadSeatDataBase(){
        ...
        await SeatDatabase.doc(this.seatId).get().then((doc)=>{
            ...
            // console.log("좌석 정보",this.timeTable);
            this.isLoad = true;
        }).catch((err)=>{
    ...
    getSeatDataOnTimeReserve(time,com){
    ...
    async doSeatReservation(time,number,uid){
        ...
        if(!this.timeTable.checkSeat(time,number)){ // 예약이 가능하다면
            this.timeTable.insertSeat(time,{seat:number, uid:uids, state:false});
    ...
    async doSeatCancel(time,number){
        if(!this.isLoad) return;
        //console.log( "최초 카페테이블 내역입니다 : \n", this.timeTable.getSeatDataFromTime(time),"\n__________________________________________________________")
