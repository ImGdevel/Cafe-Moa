...
3. getCurrentUserId를 하시면 현제 로그인한 유저의 ID를 가져오실수 있습니다.


UserDataService에 궁극적 목표는 다음과 같습니다.
1. UserData에는 [개인정보] [현제 예약정보] [찜 카페] [과거 예약내역] [평가내역] 입니다.
      - 현제 당장 구현해햐 하는 것은 [개인정보]와 [현제 예약정보] 두가지 항목입니다. 나머지는 추후 제작합니다.
...
주로 Promise 오류메게지가 그 유형입니다. (consloe.log 잘못 호출해도 Promise뜨긴함)
*/
...
export async function getUserProfile(){ //유저 데이터를 가져옵니다. 샘플: 미구현
    userId = await getCurrentUserId();
    let UserData;
