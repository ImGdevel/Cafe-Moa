...
export class CafeData{
  /**  */
  constructor(name, location, address, seatnumber, opentime, closetime, LogoImage, SeatImage) {
    ...
    this.seatImge = SeatImage;    //좌석 이미지
  ...
  loadData(data){
    ...
    this.startTime = Number(data.time.open);
    this.endTime = Number(data.time.close);
    this.seatId = data.seatId;

    this.rating = (data.rating != undefined) ? data.rating : null;  
    this.review = (data.review != undefined) ? data.review : [];
    ...
    this.notice = (data.notice != undefined) ? data.notice : null;
    (data.image != undefined) ? this.cafeImgs.update(data.image) : [];
  }

  async updateImage(){
    this.setLogoImage(await getImage("Cafe",this.id,"logo"));
    this.setSeatImage(await getImage("Cafe",this.id, "seatImage"));
  }
  ...
  * defalt는 start = 1 end = 2*/
  ...
  addRatingPoint(num){
    if(this.reviewCount == 0){
      ...
      this.rating = parseFloat((this.rating*this.reviewCount + num) / (this.reviewCount + 1));  
    }
  }
