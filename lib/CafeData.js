...
export class CafeData{
    constructor(_name, _brand , _geo_loc, _address, _number_of_seat, _start_time, _end_time) {
      ...
      this.name = _name;            //카폐이름      
      this.brand = _brand           //브랜드 (현제 미사용)
      this.location = _geo_loc;     //좌표
      this.adress = _address;       //주소
      this.count = _number_of_seat; //좌석 갯수
      this.startTime = _start_time; //오픈 시간
      this.endTime = _end_time;     //폐점 시간
      this.network = new Array();   //주변 카페 네트워크
      this.Logo = null;             //카페 로고 이미지
    ...
    addRatingPoint(num){
      this.rating = float((this.rating*this.reviewCount + num) / (this.reviewCount + 1))
    }

    getRating(){
      let rat = float(this.rating);
