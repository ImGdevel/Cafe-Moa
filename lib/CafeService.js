...
export class CafeService{
    ...
    async setLocation(){
        ...
        this.adress = (this.adress == null) ? await getAddressFromLocation(this.location) : this.adress;
    }

    async getCafeDatabase(){
        this.location = {latitude: 37.25894 ,longitude: 127.19069}
        this.adress =  {city1: "경기도" , city2: "용인시", city3:"처인구"}
        
        let queue = (await serchNearestCafe(this.location,this.adress)).allList();
    ...
    async serchNearestCafe(_loc,_city){
        ...
        let city = this.adress;
        let search_range = 0.08;
        let p_queue = new PriorityQueue();
        if(location == null){
            console.log("도시정보 없음");
            location = getAddressFromLocation(location);
        }
    ...
    sortCafeData(type){
        //카페데이터 정렬
        const DISTANCE = 1, RATING = 2;
        //console.log(this.cafeDataList)
        let sortedList = this.cafeDataList.sortDistance();
