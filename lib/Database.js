...
import { createNewSeatData, doSeatReservation, getSeatDataAll, loadSeatDataBase, ReservationService, UpdateSeatDataBase } from './ReservationService';
...
export async function testings(){
    var sapleseatId = "4DlZsYaBv78lG2D4zlVD"

    const rserviece = new ReservationService(sapleseatId);
    
    await rserviece.loadSeatDataBase();    
...
export async function addCafeDatabase(_data){
    ...
    let open = 9, close = 22;
    let seatid = await createNewSeatData(open, close);

    /*좌석 하기*/
    // let table = new TimeTable(10,20);
    // table.insertSeat(10,{seat:15,uid:"sadsad"});
    // table.insertSeat(10,{seat:20,uid:"saqwesa***d"});
    // table.insertSeat(10,{seat:30,uid:"sad___sa33qwed"});
    // table.insertSeat(10,{seat:10,uid:"123sadsad"});

    // await UpdateSeatDataBase(seatid,table);

    // await loadSeatDataBase(seatid).then((data)=>{
    //     console.log("10시",data.getSeatDataFromTime(10))
    // });
