...
export async function addCafeDatabase(_data){
    ...
    try{
        ...
        cafe_datas.seat = await createNewSeatData(0, 23);
        ...
        var id = await CafeDatabase.add({
            ...
            network: cafe_datas.getNetwork(),
            seatId: cafe_datas.getSeatId(),
            logo: cafe_datas.getLogo(),
            seatImage: cafe_datas.getSeatImage(),
        }).catch((err)=>{
        ...
        */
        ...
        //인접 데이터 세팅
        insertDataInLocation(cafe_datas.adress,cafe_datas.location,id.id);
        if(data != false){
        ...
        console.log("카페를 업로드 하지 못햇습니다 : ",err);
    }
}
